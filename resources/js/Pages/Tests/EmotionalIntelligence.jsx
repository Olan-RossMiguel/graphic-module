...
export default function EmotionalIntelligence({
    ...
    const testStorageKey = `test_inteligencia_emocional_${props?.auth?.user?.id || 'guest'}`;
    ...
    const executeSubmit = () => {
        ...
        router.post(
            '/tests/inteligencia-emocional/submit',
            { answers: allAnswers },
            {
                onSuccess: () => {
                ...
                <div className="mb-8 overflow-hidden rounded-xl bg-white shadow-sm">
                    <div className="border-b px-6 py-6 sm:px-8 sm:py-8">
                        <h1 className="mb-3 text-2xl font-bold text-gray-900 sm:text-3xl lg:text-4xl">
                            {test?.nombre || 'Test de Inteligencia Emocional'}
                        </h1>
